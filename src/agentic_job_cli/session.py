...
from agentic_job_cli.client import A0Client, A0ConnectorPluginMissingError, DEFAULT_HOST
from agentic_job_cli.config import CLIConfig, save_last_context
...
from agentic_job_cli.protocol import connector_version_warning, validate_capabilities
from agentic_job_cli.remote_exec import RemoteExecManager
...
class ConnectorSession:
    """UI-agnostic Agentic Job connector session.
    ...
    async def connect(
        ...
        *,
        ...
        if self.tools_only and not {
            ...
            raise SessionError(
                "CONTRACT_MISMATCH",
                "Agentic Job does not advertise the current Launcher gateway scopes.",
                stage="capabilities",
                exit_code=2,
            )
        ...
        if self.tools_only:
            ...
            if "launcher_gateway_control" not in hello_features:
                ...
                raise SessionError(
                    "CONTRACT_MISMATCH",
                    "Agentic Job does not advertise launcher_gateway_control.",
                    stage="websocket",
                    exit_code=2,
                )
            self.connected = True
    ...
    async def _ensure_authenticated(
        ...
        *,
        ...
        self._stage("login", "Verifying Agentic Job session...", host)
        if restore_session:
            with contextlib.suppress(Exception):
    ...
    def _require_client(self) -> A0Client:
        if self.client is None:
            raise SessionError("NOT_CONNECTED", "Not connected to Agentic Job.", stage="connection", exit_code=1)
        return self.client
