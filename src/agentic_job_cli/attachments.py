...
_HOST_UPLOAD_ROOT_ENV = "AJ_CONNECTOR_UPLOADS_HOST_ROOT"
_CONTAINER_UPLOAD_ROOT_ENV = "AJ_CONNECTOR_UPLOADS_CONTAINER_ROOT"
_DEFAULT_CONTAINER_UPLOAD_ROOT = "/aj/usr/uploads"
...
def _host_upload_root() -> Path:
    ...
    raise AttachmentError(
        "No Agentic Job upload directory is configured. Use the HTTP upload path instead."
    )
...
def _host_path_from_container_root(container_root: str, *, volume_root: Path) -> Path:
    ...
    try:
        relative_root = PurePosixPath(normalized).relative_to("/aj")
    except ValueError:
