"""A0Client for the current Agentic Job connector API over HTTP + `/ws`."""
...
from agentic_job_cli.attachments import AttachmentRef, AttachmentUpload, remote_upload_path

_PLUGIN_API = "/api/plugins/_aj_connector/v1"
# Agentic Job's installer defaults to the first free port starting at 5080.
DEFAULT_HOST = "http://localhost:5080"
PROTOCOL_VERSION = "aj-connector.v1"
_SOCKET_IO_PATH = "/socket.io"
WS_NAMESPACE = "/ws"
WS_HANDLER = "plugins/_aj_connector/ws_connector"
...
class A0ConnectorPluginMissingError(RuntimeError):
    """HTTP 404 on the connector API — the _aj_connector plugin is not loaded on Agentic Job."""
...
class A0Client:
    """Client for communicating with a running Agentic Job instance."""
    ...
    def persist_session(self, host: str) -> None:
        from agentic_job_cli.config import save_persisted_session

        save_persisted_session(host, self._session_cookie_records())

    def restore_session(self, host: str) -> bool:
        from agentic_job_cli.config import load_persisted_session
    ...
    def clear_persisted_session(self, host: str) -> None:
        from agentic_job_cli.config import delete_persisted_session
    ...
    async def fetch_image(
        ...
        *,
        ...
        """Load one same-origin image through Agentic Job's authenticated session."""
    ...
    def _validate_image_path(self, path: str) -> None:
        if (
            ...
            or not path.startswith("/aj/")
            or path.startswith("//")
            or "?" in path
            or "#" in path
        ):
            raise A0ProtocolError("Image path must be a safe Agentic Job path.")
    ...
    async def _read_image_response(
        ...
        *,
        ...
        if self._is_login_redirect(response):
            raise A0ProtocolError("Image request requires an authenticated Agentic Job session.")
    ...
    async def _probe_socketio_transport(self) -> None:
        ...
        try:
            ...
            raise A0WebSocketConnectionError(
                ...
                f"{probe_url}?transport=polling&EIO=4. Ensure Agentic Job is running and any "
        ...
        if response.status_code != 200:
            raise A0WebSocketConnectionError(
                ...
                "Ensure Agentic Job is running and any reverse proxy forwards /socket.io unchanged "
    ...
    def _format_namespace_rejection_error(self, exc: BaseException | None = None) -> str:
        ...
        guidance = (
            ...
            "AGENT_ZERO_HOST exactly matches the Agentic Job URL (for example localhost vs "
    ...
    async def fetch_capabilities(self) -> dict[str, Any]:
        ...
        if response.status_code == 404:
            raise A0ConnectorPluginMissingError(
                "HTTP 404 — the builtin _aj_connector plugin is not available on this Agentic Job server.\n"
                "\n"
                "The web UI can work while this endpoint is missing: the CLI needs the plugin.\n"
                "On a remote host, update Agentic Job before retrying."
            )
    ...
    async def fetch_csrf_token(self) -> str:
        ...
        if self._is_login_redirect(response):
            raise A0ProtocolError("CSRF token request requires an authenticated Agentic Job session.")
        if response.status_code >= 400:
    ...
    async def upload_attachments(self, uploads: list[AttachmentUpload]) -> list[AttachmentRef]:
        ...
        if self._is_login_redirect(response):
            raise A0ProtocolError("Upload requires an authenticated Agentic Job session.")
        if response.status_code >= 400:
    ...
    async def goal_action(
        ...
        **payload: Any,
        ...
        if self._is_login_redirect(response):
            return {
                "ok": False,
                "message": "Goal API requires an authenticated Agentic Job session.",
                "status_code": 401,
            }
        if response.status_code >= 400:
            return {
