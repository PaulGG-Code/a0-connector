...
from agentic_job_cli.config import CLIConfig, load_config
from agentic_job_cli.headless.commands import (
    HeadlessCommandResult,
    command_may_start_agent,
    dispatch_headless_command,
    is_headless_command,
)
from agentic_job_cli.headless.renderer import HeadlessRenderer, make_renderer
from agentic_job_cli.instance_discovery import discover_local_instances
...
class HeadlessRunner:
    ...
    async def _connect_with_auth_retry(self, host: str) -> bool:
        username = os.environ.get("AJ_USERNAME", "").strip()
    ...
    async def _resolve_host(self) -> str:
        ...
        if result.status == "ready" and len(result.instances) > 1:
            raise SessionError(
                "HOST_REQUIRED",
                "Multiple local Agentic Job instances were found; pass --host URL.",
                stage="discovery",
                exit_code=EXIT_CONNECT_OR_AUTH,
            )
        detail = result.detail or "No local Agentic Job Docker instance was discovered."
        raise SessionError(
            "HOST_REQUIRED",
    ...
    def _require_session(self) -> ConnectorSession:
        if self.session is None:
            raise SessionError("NOT_CONNECTED", "Not connected to Agentic Job.", exit_code=EXIT_ERROR)
        return self.session
