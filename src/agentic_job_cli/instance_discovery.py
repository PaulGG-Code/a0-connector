...
def _container_name(container: Mapping[str, Any]) -> str:
    ...
    return _stringify(config.get("Hostname")) or "Agentic Job"
...
def _mount_targets_aj(container: Mapping[str, Any]) -> bool:
    ...
    if isinstance(mounts, list):
        for mount in mounts:
            ...
            if destination == "/aj" and (not mount_type or mount_type == "bind"):
                return True

    host_config = _mapping(container.get("HostConfig"))
    for bind in _string_list(host_config.get("Binds")):
        parts = bind.split(":")
        if len(parts) >= 2 and parts[1].rstrip("/") == "/aj":
            return True
    return False
...
def _image_signal(container: Mapping[str, Any]) -> bool:
    ...
    return "agentic-job" in image_text or "agent-zero" in image_text


def _looks_like_agent_zero(container: Mapping[str, Any]) -> bool:
    return _image_signal(container) or _command_signal(container) or _mount_targets_aj(container)
...
async def _discover_with_docker_client(client: httpx.AsyncClient, *, source: str) -> DiscoveryResult:
    ...
    if instances:
        ...
        return DiscoveryResult(
            status="ready",
            instances=instances,
            detail=f"Found {count} local Agentic Job endpoint{'s' if count != 1 else ''}.",
        )

    return DiscoveryResult(
        status="empty",
        detail="No running Agentic Job Docker WebUI endpoints were detected.",
    )
