...
_PYTHON_CODE_ENV = "AJ_PY_CODE"
_NODE_CODE_ENV = "AJ_NODE_CODE"
...
def _build_python_command(code: str) -> str:
    ...
    decoder = (
        "import base64, os; exec(compile(base64.b64decode(os.environ['AJ_PY_CODE'])."
        "decode('utf-8'), '<aj-remote>', 'exec'))"
    )
...
def _build_node_command(code: str) -> str:
    encoded = base64.b64encode(code.encode("utf-8")).decode("ascii")
    decoder = (
        "const src = Buffer.from(process.env.AJ_NODE_CODE, 'base64').toString('utf8'); "
        "(0, eval)(src);"
    )
