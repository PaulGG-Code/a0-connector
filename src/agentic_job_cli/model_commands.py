...
from agentic_job_cli.model_config import (
    apply_model_switcher_state,
    coerce_model_config,
    collect_provider_options,
)
from agentic_job_cli.state_sync import model_switcher_signature
from agentic_job_cli.screens.model_presets import ModelPresetsResult, ModelPresetsScreen
from agentic_job_cli.screens.model_runtime import ModelRuntimeResult, ModelRuntimeScreen
from agentic_job_cli.widgets.model_switcher_bar import ModelSwitcherBar
...
async def cmd_models(app: AgentZeroCLI, *, focus_target: str = "main") -> None:
    ...
    if not isinstance(default_preset, dict):
        app._show_notice("Agentic Job did not provide its Default model preset.", error=True)
        return
