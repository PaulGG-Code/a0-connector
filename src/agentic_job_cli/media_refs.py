    ...
    for key in ("uri", "aj_path", "path"):
        ...
        if isinstance(value, str):
            appended = _append_reference_candidate(
                ...
                raw_value=value,
                base_url=base_url,
                allow_direct_aj_path=key in {"aj_path", "path"},
            )
            if appended:
                return
...
def _append_reference_candidate(
    ...
    *,
    ...
    raw_value: str,
    base_url: str,
    allow_direct_aj_path: bool = False,
) -> bool:
    normalized = None
    if allow_direct_aj_path and raw_value.startswith("/aj/"):
...
def _safe_aj_path(value: str) -> str | None:
    path = unquote(value)
    if not path.startswith("/aj/") or "?" in path or "#" in path:
        return None
