...
from agentic_job_cli import computer_use_wayland as _builtin_computer_use_wayland  # noqa: F401
...
_HOST_ARTIFACT_ROOT_ENV = "AJ_COMPUTER_USE_HOST_ARTIFACT_ROOT"
_CONTAINER_ARTIFACT_ROOT_ENV = "AJ_COMPUTER_USE_CONTAINER_ARTIFACT_ROOT"
_DEBUG_ENV = "AJ_COMPUTER_USE_DEBUG"
_DEBUG_LOG_ENV = "AJ_COMPUTER_USE_DEBUG_LOG"
_DEFAULT_CONTAINER_ARTIFACT_ROOT = "/aj/tmp/_aj_connector/computer_use"
...
def _default_host_artifact_root(_container_root: str) -> Path:
    ...
    return Path(tempfile.gettempdir()) / "_aj_connector" / "computer_use"
...
class ComputerUseManager:
    ...
    def _debug(self, event: str, **fields: object) -> None:
        if not self._debug_enabled:
            return
        line = f"[aj computer_use] {_debug_timestamp()} {event}"
        if fields:
            formatted = " ".join(
        ...
        *,
        ...
        while True:
            ...
            if accessibility != "granted":
                message = "Allow Agentic Job Launcher in macOS Accessibility settings."
            ...
            if screen_recording != "granted":
                message = "Allow Agentic Job Launcher in macOS Screen Recording settings."
                snapshot = self._set_permission_setup(
                    "screen_recording_required",
                    accessibility="granted",
                    screen_recording=screen_recording,
                ...
                if screen_restart_candidate:
                    return self._set_permission_setup(
                        ...
                        message="Restart Agentic Job Launcher to finish Screen Recording setup.",
                    )
