...
from agentic_job_cli.host_browser_session import HostBrowserSession, ProfileLockedError
...
def normalize_host_browser_profile_mode(value: object, *, default: str = "") -> str:
    ...
    if not normalized:
        return default
    if normalized in {"agent", "clean", "clean_agent", "a0", "aj", "dedicated"}:
        return "agent"
...
class HostBrowserManager:
    ...
    def status_text(self) -> str:
        ...
        if not status["supported"]:
            if status["can_prepare"]:
                return (
                    "Host browser can be prepared automatically when Agentic Job first uses "
                    "the Browser tool."
                )
            return f"Host browser unsupported: {status['support_reason']}"
        if status["status"] == "disabled":
            return "Host browser is disabled. Use /browser host on to advertise it to Agentic Job."
        profile_text = (
    ...
    def _selected_existing_profile(
        ...
        *,
        ...
        if family or profile_path or profile_label:
            for profile in profiles:
                if profile.is_remote_debugging or is_aj_managed_family(profile.family):
                    continue
                if family and profile.family != family:
                    continue
                if profile_path and profile.profile_path_display != profile_path:
        ...
        return next(
            (
                profile
                for profile in profiles
                if not profile.is_remote_debugging and not is_aj_managed_family(profile.family)
            ),
            None,
        )
    ...
    def _selected_agent_profile(
        ...
        *,
        ...
        agent_profiles = [
            profile
            for profile in profiles
            if not profile.is_remote_debugging and is_aj_managed_family(profile.family)
        ]
        if family or profile_path or profile_label:
    ...
    async def handle_op(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self.enabled:
            return self._error(op_id, "HOST_BROWSER_DISABLED", "Host browser is disabled in the AJ CLI.")
        try:
        ...
        if lock.locked and context_id not in self._sessions and active_context != RELAUNCH_CONTEXT_ID:
            if active_context:
                return self._error(
                    ...
                    (
                        "Host browser is already controlled by another Agentic Job browser context. "
            ...
            return self._error(
                ...
                (
                    ...
                    "Run /browser relaunch after closing that browser to give AJ explicit control."
                ),
    ...
    def _profile_matches_mode(profile: BrowserProfile, mode: str) -> bool:
        if not mode:
            return True
        if mode == "agent":
            return not profile.is_remote_debugging and is_aj_managed_family(profile.family)
        return profile.is_remote_debugging or not is_aj_managed_family(profile.family)
    ...
    def _matching_remote_profile(
        ...
        *,
        ...
        if not family or is_aj_managed_family(family) or is_remote_debugging_family(family):
            if profile_label:
    ...
    def _support_reason(self, profile: BrowserProfile | None = None) -> str:
        ...
        if not self._has_playwright():
            return (
                f"Browser support is incomplete in the AJ CLI host environment ({sys.executable}). "
                "Use the current Browser setup action, or run /browser repair in the AJ CLI. "
                "The Playwright runtime inside the Agentic Job Docker container is used by the "
