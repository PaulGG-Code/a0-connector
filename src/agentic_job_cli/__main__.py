...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aj",
        description="Terminal chat interface for Agentic Job.",
        epilog=(
    ...
    parser.add_argument(
        "--version",
        action="store_true",
        help="Print the installed aj version and exit.",
    )
    ...
    connection_options.add_argument(
        ...
        metavar="URL",
        help=(
            "Prefill the host picker with an Agentic Job base URL. "
    ...
    connection_options.add_argument(
        "--no-auto-connect",
        action="store_true",
        help="Do not auto-connect when Docker discovery finds exactly one local Agentic Job instance.",
    )
    connection_options.add_argument(
        "--no-docker-discovery",
        action="store_true",
    ...
    chat_options.add_argument(
        ...
        help=(
            ...
            "Defaults to AGENT_ZERO_DEFAULT_CONTEXT_ID, AJ_DEFAULT_CHAT, "
    ...
    subparsers.add_parser(
        "update",
        help="Update the installed aj tool and exit.",
    )
    ...
    headless_connection.add_argument(
        ...
        dest="headless_host",
        metavar="URL",
        help="Agentic Job base URL. Defaults to AGENT_ZERO_HOST or single-instance Docker discovery.",
    )
    ...
    gateway.add_argument(
        ...
        dest="gateway_host",
        metavar="URL",
        help="Agentic Job base URL. Defaults to AGENT_ZERO_HOST or the saved host.",
    )
    ...
    gateway.add_argument("--host-label", default="", metavar="LABEL", help="Human-readable host name shown in Agentic Job.")
...
def _run_app(
    *,
    ...
    from agentic_job_cli.config import load_config
    from agentic_job_cli.image_render import initialize_image_renderer
    from agentic_job_cli.textual_compat import install_textual_linux_input_decoder_guard
...
def _run_self_update() -> int:
    from agentic_job_cli.self_update import run_self_update_handoff
...
def _run_headless(
    *,
    ...
    from agentic_job_cli.config import load_config
    from agentic_job_cli.headless.runner import HeadlessOptions, run_headless
...
def _run_gateway(
    *,
    ...
    from agentic_job_cli.config import load_config
    from agentic_job_cli.gateway import gateway_options, run_gateway
