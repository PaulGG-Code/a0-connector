...
class InstalledPluginsScreen(ModalScreen[None]):
    ...
    def _sync_empty(self) -> None:
        ...
        if not self._entries:
            empty.update("No installed plugins were reported by Agentic Job Core.")
        elif self._filter:
    ...
    def action_toggle_selected(self) -> None:
        ...
        if not entry.toggleable:
            reason = entry.protected_reason or "Agentic Job marks this plugin as protected."
    ...
    async def _toggle_entry(self, entry: InstalledPluginEntry) -> None:
        ...
        self._set_status(f"{entry.title} {state}. Changes may affect new Agentic Job runs.")
