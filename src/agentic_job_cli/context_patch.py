...
def apply_context_patch(content: str, patch_text: str) -> str:
    ...
    cursor = 0
    for hunk in hunks:
        cursor = _apply_hunk(lines, hunk, cursor)
    return "\n".join(lines)
...
def _extract_single_file_body(patch_text: str) -> list[str]:
    ...
    for line in lines[1:-1]:
        if line.startswith("*** Update File:"):
            if update_count:
                raise ContextPatchError("patch_text may update only one file per operation")
            in_update = True
            update_count += 1
            continue
...
def _apply_hunk(lines: list[str], hunk: _Hunk, cursor: int) -> int:
    ...
    if not old_lines:
        ...
        return insert_at + len(new_lines)
