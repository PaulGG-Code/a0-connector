...
from agentic_job_cli.instance_discovery import DiscoveredInstance, DiscoveryStatus
from agentic_job_cli.widgets.chat_log import build_agent_zero_banner_widget
...
class SplashHostPanel(Vertical):
    ...
    def __init__(self) -> None:
        ...
        self._title = Static("Local Agentic Job instances", classes="splash-panel-title")
        self._copy = Static(
            "Detected Agentic Job WebUI endpoints. Manual URL entry is available below.",
        ...
        self._manual_copy = Static(
            "Use this for remote Agentic Job hosts or anything Docker cannot see. Standard ports are optional.",
            classes="splash-panel-copy",
        )
        self._hint = Static(
    ...
    def _status_message(self) -> Text | str:
        ...
        if status == "loading":
            return Text("Checking Docker for local Agentic Job instances...", style="#9aa7b4")
        if status == "ready":
            noun = "endpoint" if count == 1 else "endpoints"
            return Text(f"{count} Detected AJ {noun} ready to connect.", style="#79d18a")
        if status == "empty":
            message = detail or "No local Agentic Job Docker instances were found."
            return Text(f"{message} Install Agentic Job: http://agent-zero.ai", style="#9aa7b4")
...
class SplashLoginPanel(Vertical):
    ...
    def __init__(self) -> None:
        ...
        self._copy = Static(
            "Sign in to the Agentic Job instance below.",
            classes="splash-panel-copy",
        )
    ...
    def _render_target_context(self) -> None:
        ...
        if self._login_error:
            ...
            if self._target_detected_label:
                summary = Text.assemble(
                    ("Detected AJ instance ", "bold #79d18a"),
        ...
        copy_text = (
            "Use the same username and password you use in the Agentic Job Web UI."
            if self._target_detected_label
            else "Login with the Agentic Job endpoint below."
        )
