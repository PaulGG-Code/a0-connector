...
- App-level orchestration remains in `src/agentic_job_cli/app.py` and command modules.
- Visual constants that require layout styling belong in `src/agentic_job_cli/styles/app.tcss`.
...
- Transcript renderable caches must use AJ-owned attribute names and must not
