...
from agentic_job_cli.client import A0Client, DEFAULT_HOST
from agentic_job_cli.attachments import (
    AttachmentError,
    create_clipboard_image_upload,
)
from agentic_job_cli.clipboard import (
    copy_text_to_windows_clipboard,
    should_use_native_windows_clipboard,
)
from agentic_job_cli.computer_use import ComputerUseManager
from agentic_job_cli.host_browser import HostBrowserManager
from agentic_job_cli.commands import CommandAvailability, CommandSpec
from agentic_job_cli.config import CLIConfig, load_config, save_last_context
from agentic_job_cli.instance_discovery import DiscoveryResult, discover_local_instances
from agentic_job_cli.image_render import ImageRenderer
from agentic_job_cli.image_store import ImageStore, ImageUnavailableError
from agentic_job_cli.media_refs import ImageReference
from agentic_job_cli.remote_exec import PythonTTYManager
from agentic_job_cli.remote_files import RemoteFileUtility
from agentic_job_cli.project_utils import (
    normalize_project_list,
    normalize_project_summary,
    project_color,
    project_name,
)
from agentic_job_cli.widgets.command_palette import (
    AgentCommandPalette,
...
from agentic_job_cli.widgets.chat_log import ChatLog
from agentic_job_cli.widgets.image_entry import ImageEntry
from agentic_job_cli.model_commands import (
    cmd_model_presets,
    cmd_models,
    set_model_preset,
...
from agentic_job_cli.token_usage import (
    start_token_refresh,
    stop_token_refresh,
    refresh_token_usage,
)
...
class AgentZeroCLI(App):
    """Agentic Job CLI - terminal-native connector shell."""

    CSS_PATH = "styles/app.tcss"
    TITLE = "Agentic Job CLI"
    ...
    def get_system_commands(self, screen) -> Iterable[SystemCommand]:
        ...
        for command in self._server_commands:
            ...
            description = str(command.get("description") or "Run Agentic Job command.").strip()
        ...
        yield SystemCommand(
            "Browser: Use Host",
            "Run Browser through AJ CLI against your Chromium-family browser.",
        ...
        yield SystemCommand(
            ...
            "Run Browser inside the Agentic Job Docker/container browser.",
            lambda: self.run_worker(
    ...
    def _build_command_registry(self) -> tuple[CommandSpec, ...]:
        return (
            ...
            CommandSpec(
                "/profile",
                (),
                "Manage, select, or quickly create an Agentic Job Core profile.",
            ...
            CommandSpec(
                "/plugins",
                (),
                "Open the installed-only Agentic Job plugin toggle view.",
    ...
    async def _open_profile_menu(self) -> None:
        ...
        if not options and "agent_editor" not in self.connector_features:
            self._show_notice("No agent profiles are available from Agentic Job Core.", error=True)
            return
    ...
    async def _activate_skill(self, skill: Mapping[str, Any]) -> bool:
        if not self.current_context:
            ...
            return False
        if not self._skills_available():
            self._show_notice("Skill invocation is unavailable on this Agentic Job instance.", error=True)
            return False
    ...
    async def _set_computer_use_enabled(self, enabled: bool) -> None:
        ...
        if synced:
            ...
            self._show_notice(
                "Computer use changed locally, but Agentic Job did not acknowledge "
    ...
    async def action_toggle_remote_file_mode(self) -> None:
        ...
        if synced:
            ...
            self._show_notice(
                "Local access changed locally, but Agentic Job did not acknowledge "
                f"the update: {self._remote_tool_metadata_error}",
                error=True,
            )
