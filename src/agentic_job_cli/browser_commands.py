...
_CONTENT_POLICY_NOTICE = (
    "For Browser model-use settings, visit Agentic Job WebUI > Browser settings to choose "
...
async def _cmd_browser_runtime(app: "AgentZeroCLI", runtime_backend: str) -> None:
    if not _browser_runtime_config_available(app):
        app._show_notice(
            "This Agentic Job server does not support CLI Browser mode changes.",
            error=True,
        )
        return
    ...
    if runtime_backend == "host_required":
        ...
        if not await app._refresh_remote_tool_metadata():
            app._show_notice(
                "Host browser changed locally, but Agentic Job did not acknowledge "
...
async def _sync_browser_runtime_for_host_toggle(
    ...
    *,
    ...
    if not app.current_context:
        return (
            None,
            "Open or create a chat context before syncing Browser mode in Agentic Job.",
            False,
        )
    if app.agent_active:
        return (
            None,
            "Wait for the current run to finish before syncing Browser mode in Agentic Job.",
            False,
        )
...
async def _cmd_browser_profile(app: "AgentZeroCLI", args: list[str]) -> None:
    ...
    if synced:
        ...
        app._show_notice(
            f"{message} Agentic Job did not acknowledge the update: {app._remote_tool_metadata_error}",
            error=True,
        )
...
async def _cmd_browser_select(app: "AgentZeroCLI", args: list[str]) -> None:
    ...
    if not _browser_runtime_config_available(app):
        app._show_notice(
            "This Agentic Job server does not support CLI host-browser selection.",
            error=True,
        )
        return
    if not app.current_context:
...
async def _cmd_browser_relaunch(app: "AgentZeroCLI") -> None:
    ...
    if synced:
        ...
        app._show_notice(
            "Host browser relaunched locally, but Agentic Job did not acknowledge "
            f"the update: {app._remote_tool_metadata_error}",
            error=True,
    )
...
def _format_host_browser_choices(choices: list[dict[str, str]]) -> str:
    lines = [
        ...
        "0 auto - Automatic (AJ CLI chooses)",
    ]
...
def _host_browser_selection_label(selection: str, choices: list[dict[str, str]]) -> str:
    if not selection:
        return "Automatic (AJ CLI chooses)"
...
async def _ensure_playwright_dependency(app: "AgentZeroCLI") -> bool:
    ...
    app._show_notice(
        "Host browser needs Python Playwright in the AJ CLI environment. "
    ...
    if result.get("installed"):
        app._show_notice(
            ...
            "AJ will use your installed Chromium-family browser; no bundled browser was installed."
        )
    else:
