...
# owned by Agentic Job's Browser plugin and is delivered over the connector protocol.
...
HOST_BROWSER_ARTIFACT_ROOT_ENV = "AJ_HOST_BROWSER_ARTIFACT_ROOT"
DEFAULT_HOST_BROWSER_ARTIFACT_ROOT = Path(tempfile.gettempdir()) / "_aj_connector" / "host_browser"
PLAYWRIGHT_PYTHON_PACKAGE = "playwright"
HOST_BROWSER_OZONE_PLATFORM_ENV = "AJ_HOST_BROWSER_OZONE_PLATFORM"
HOST_BROWSER_REMOTE_DEBUGGING_ENDPOINTS_ENV = "AJ_HOST_BROWSER_REMOTE_DEBUGGING_ENDPOINTS"
...
def normalize_url(value: str) -> str:
    ...
    if not _URL_SCHEME_RE.match(raw):
        raise ValueError(
            "Host browser navigation requires an Agentic Job-normalized URL with a scheme."
        )
    return raw
...
def _with_a0_managed_candidates(candidates: list[BrowserCandidate]) -> list[BrowserCandidate]:
    ...
    for candidate in candidates:
        ...
        expanded.append(
            BrowserCandidate(
                family=f"{candidate.family}-aj",
                label=f"{candidate.label} (AJ controlled profile)",
...
def a0_managed_user_data_dir(family: str) -> Path:
    ...
    system = platform.system()
    if system == "Darwin":
        root = Path.home() / "Library/Application Support/AJ/Browser Profiles"
    elif system == "Windows":
        local_app_data = Path(os.environ.get("LOCALAPPDATA") or Path.home() / "AppData/Local")
        root = local_app_data / "AJ/Browser Profiles"
    else:
        root = Path(os.environ.get("XDG_DATA_HOME") or Path.home() / ".local/share") / "aj/browser-profiles"
    return root / family_slug


def is_aj_managed_family(family: str) -> bool:
    return str(family or "").strip().lower().endswith("-aj")
...
def discover_profiles(candidate: BrowserCandidate) -> list[BrowserProfile]:
    ...
    if is_aj_managed_family(candidate.family):
        return [
            BrowserProfile(
...
def remote_debugging_restriction_reason(profile: BrowserProfile) -> str:
    if profile.is_remote_debugging:
        return ""
    if is_aj_managed_family(profile.family):
        return ""
    ...
    if (
        ...
        managed_family = f"{profile.family}-aj"
        return (
            ...
            f"AJ-controlled local profile with /browser profile "
...
def remote_debugging_enable_hint() -> str:
    return (
        f"Open {REMOTE_DEBUGGING_ENABLE_URL} in the browser you want Agentic Job to use, "
...
def is_default_user_data_dir(family: str, user_data_dir: Path | str) -> bool:
    normalized_family = str(family or "").strip().lower().removesuffix("-aj")
